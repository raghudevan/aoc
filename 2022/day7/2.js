...
export function runTests() {
  ...
  return assert(testResult, 24933642);
}
...
function coreLogic(input) {
  ...
  const diskUsed = dir.size;
  const diskSize = 70000000;
  const freeSpaceNeeded = 30000000;
  const needToFreeUp = freeSpaceNeeded - (diskSize - diskUsed);

  const smallestSizeDirToDelete = _.chain(dirs)
    .sortBy("size")
    .find(({ size }) => size > needToFreeUp)
    .get("size")
    .value();

  return smallestSizeDirToDelete;
}
