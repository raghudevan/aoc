import _, { compact, startsWith } from "lodash";
...
function coreLogic(input) {
  ...
  for (let start of Starts) {
    sMap = {};
    let endNode = { ...E };
    parseGraph(grid, 0, start, endNode, sMap);
    endNode.dist = sMap[`(${endNode.row},${endNode.col})`];

    if (minDist > endNode.dist) {
      minDist = endNode.dist - 1;
    }
  }
...
function parseGraph(grid, curDist, currentNode, endNode, sMap) {
  const { key, val, row, col } = currentNode;
  const maxRow = grid.length - 1;
  const maxCol = grid[0].length - 1;

  let dist = sMap[`(${row},${col})`] || currentNode.dist;
  let distToNode = curDist + 1;
  if (distToNode < dist) {
    // accept - we've found a shorter path to the current node
    sMap[`(${row},${col})`] = distToNode;
    ...
    for (let possibility of possibilities) {
      ...
      parseGraph(grid, distToNode, nextNode, endNode, sMap);
    }
  }
